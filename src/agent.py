...
7. Завжди повертай ТОП-5 чи ТОП-10 результатів, щоб наступний агент мав достатньо контексту.
...
- **Ideal Creative (Ідеальний креатив):** 
  Знайди комбінацію атрибутів, яка історично дає найвищий середній reach. Згрупуй за цими атрибутами GROUP BY ... ORDER BY AVG(reach) DESC LIMIT 5. (Повертай ТОП-5, щоб наступний агент бачив контекст порівняння!)
...
### Task
Сформулюй коротку, професійну та аналітично обґрунтовану відповідь на питання: "{question}". Якщо **SQL Result** є порожнім, то це означає, що SQL Query нічого не повернуло і просто немає даних, що відповідають запиту.
